...
import { Api,ShoppingCart,ArrowCircleRight , Favorite } from "@mui/icons-material";
...
function Products() {
    ...
    return (
        <div className="max-w-screen-2xl mx-auto grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 xl:gap-10 px-4">
            {
                products.map((item) => (
                    <div key={item.id} className="bg-white h-auto border-[1px] border-gray-200 py-8 z-30 hover:border-tranparent shadow-none hover:shadow-textShadow duration-200 relative flex flex-col gap-5">
                        <span className="text-xs capitalize italic absolute top-2 right-2 text-gray-500">{item.category}</span>
                        <div className="w-full h-auto flex items-center justify-center relative group">
                            <img className="w-52 h-64 object-contain" src={item.image} alt="productImage"></img>
                            <ul className="w-full h-36 bg-gray-100 absolute bottom-[-170px] flex flex-col items-end justify-center gap-2 font-titleFont px-2 border-1 border-r group-hover:bottom-0 duration-700">
                                <li className="productli">
                                    Compare{" "}<span><Api/></span>
                                </li>
                                <li className="productli">
                                    Add to Cart{" "}<span><ShoppingCart/></span>
                                </li>
                                <li className="productli">
                                    View Details{" "}<span><ArrowCircleRight/></span>
                                </li>
                                <li className="productli">
                                    Add to wish list{" "}<span><Favorite/></span>
                                </li>
                            </ul>
                        </div>
                        <div className="px-4 z-10 bg-white">
                            <div className="flex items-center justify-between">
                                <h2 className="font-titleFont tracking-wide text-lg text-amazon_light font-medium">{item.title.substring(0, 20)}</h2>
